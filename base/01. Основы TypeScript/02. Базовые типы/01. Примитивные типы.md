...
Для условий, флагов и логических операций.

```ts
let isActive: boolean = true;
let isLoggedIn: boolean = false;
if (isActive) {
    console.log("User is active"); // Вывод: User is active
}
```
...
Ошибка при присвоении небулевого значения:

```ts
let flag: boolean = 1; // Error: Type '1' is not assignable to type 'boolean'
```
...
Представляет отсутствие значения или намеренное "пустое" значение.

### Использование:

Для обозначения, что переменная не содержит данных.

```ts
let data: null = null;
let user: { name: string } | null = null;
console.log(data); // Вывод: null
```

### Особенности:

В TypeScript с флагом "strictNullChecks": true (рекомендуется) null нельзя присвоить переменной другого типа без явного
указания:
...
Для разрешения null используйте union type:
